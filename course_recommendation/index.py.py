...
class ResultFunctions:
	...
	def recommend_course(self):
		...
		recommended_course, total_score = max(results, key=lambda x: x[1])
		max_possible_score = len(results) * 10  
		percentage = (total_score / max_possible_score) * 100
		recommendation_text = f"{first_name}, we recommend the course {recommended_course} to you as you got {percentage:.2f}% !"
		label_recommendation = tk.Label(self.root, text=recommendation_text, font=("Helvetica", 16, "bold"))
		label_recommendation.pack(pady=(100, 10))
			
		date_now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
		self.db.execute("INSERT INTO result (user_id, course_name, date) VALUES (%s, %s, %s)",
						(self.user_id, recommended_course, date_now))
		button_start_exam = tk.Button(self.root, text="Start Exam Again", bg="light blue",width = 15, command=self.start_exam_again)
		button_start_exam.pack(pady=(20, 0))

		# Button para mag-logout
		button_logout = tk.Button(self.root, text="Logout", bg="red", command=self.logout)
		button_logout.pack(pady=(20, 0))
